...
describe('EmailOtpService', () => {
  ...
  it('generateOtp: revokes old, creates and sends email', async () => {
    ...
    const expiresAt = new Date(Date.now() + EmailOtpConstants.DEFAULTS.EXPIRATION_MINUTES * 60 * 1000);
    repo.create.mockImplementation(async (email, code, exp, userId) => ({
      id: 1,
      email,
      code,
      expiresAt: exp,
  ...
  it('verifyOtp: wrong code increments attempts and throws invalid', async () => {
    ...
    repo.incrementAttempts.mockResolvedValue({ id: 1, email: 'user@example.com', code: '123456', expiresAt: new Date(), attempts: 1, used: false, createdAt: new Date(), userId: 7 } as any);
  ...
  it('verifyOtp: reaches max attempts and throws locked', async () => {
    ...
    repo.getLatestActiveOtp.mockResolvedValue({ id: 2, email: 'user@example.com', code: '123456', expiresAt: new Date(Date.now() + 60_000), attempts: nearMax, used: false, createdAt: new Date(), userId: 7 } as any);
    repo.incrementAttempts.mockResolvedValue({ id: 2, email: 'user@example.com', code: '123456', expiresAt: new Date(), attempts: nearMax + 1, used: false, createdAt: new Date(), userId: 7 } as any);
  ...
  it('verifyOtp: correct code marks as used', async () => {
    repo.getLatestActiveOtp.mockResolvedValue({ id: 3, email: 'user@example.com', code: '123456', expiresAt: new Date(Date.now() + 60_000), attempts: 0, used: false, createdAt: new Date(), userId: 7 } as any);
    repo.markAsUsed.mockResolvedValue({ id: 3, email: 'user@example.com', code: '123456', expiresAt: new Date(), attempts: 0, used: true, createdAt: new Date(), userId: 7 } as any);
